...
RED.nodes.registerType("modbus-queue-info",{category:"modbus",color:"#E9967A",defaults:{name:{value:""},topic:{value:""},unitid:{value:""},queueReadIntervalTime:{value:1e3,validate:RED.validators.number()},lowLowLevel:{value:25,validate:RED.validators.number()},lowLevel:{value:75,validate:RED.validators.number()},highLevel:{value:150,validate:RED.validators.number()},highHighLevel:{value:300,validate:RED.validators.number()},server:{type:"modbus-client",required:!0},errorOnHighLevel:{value:!1},showStatusActivities:{value:!0},updateOnAllQueueChanges:{value:!1}},inputs:1,outputs:1,align:"right",icon:"modbus.png",paletteLabel:"Modbus-Queue-Info",label:function(){return this.name||"Modbus Queue Info"}})
...
id='node-input-updateOnAllQueueChanges' style='max-width:30px'> </div> <hr> <div class='form-row'> <label 
...
Modbus TCP/Serial queue information node.</p> <p>Unit-Id (0..255 tcp | 1..247 serial)</p> <p>Use inject of <pre>
msg.resetQueue = true</pre> to reset the queue.</p> <p>Reset on high level to connect with the catch node</p> <p><pre>
...
"Error on high level" with that option the node sends an error if high level is reached and on high high level.</p> <p>
With reaching the high level there comes a warning and with the high high an error for work with the catch node</p> <p>
Output: all queue information - use complete msg with debug</p> </script> 